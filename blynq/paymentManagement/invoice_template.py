...
class BlynQInvoice(SimpleDocTemplate):
    default_pdf_info = PDFInfo(title='Invoice', author='Nipun Edara', subject='Invoice')
    precision = None
    ...
    def __init__(self, invoice_path, pdf_info=None, precision='0.01'):
        ...
        self._defined_styles.add(
            ParagraphStyle('LeftHeading1', parent=self._defined_styles.get('Heading1'), alignment=TA_LEFT)
        )
        self._defined_styles.add(
    ...
    def _attribute_to_table_data(self, instance, attribute_verbose_name_list, include_label=True):
        data = []
    ...
    def _service_provider_data(self):
        if isinstance(self.service_provider_info, ServiceProviderInfo):
            ...
            return self._attribute_to_table_data(self.service_provider_info, props, include_label=False)
    ...
    def _build_service_provider_info(self):
        ...
        if service_provider_info_data:
            self._story.append(Paragraph('', self._defined_styles.get('LeftHeading1')))
            self._story.append(SimpleTable(service_provider_info_data, horizontal_align='LEFT'))
    ...
    def _client_info_data(self):
        ...
        return self._attribute_to_table_data(self.client_info, props, include_label=False)

    def _build_client_info(self):
        # ClientInfo
        client_info_data = self._client_info_data()
        if client_info_data:
            self._story.append(Paragraph('Billed To', self._defined_styles.get('RightHeading1')))
        ...
        if isinstance(self.service_provider_info, ServiceProviderInfo) and isinstance(self.client_info, ClientInfo):
            ...
            table_data = [
                [
                    ...
                    '\n',
                    Paragraph('Billed to', self._defined_styles.get('Heading1')), ''
                ]
            ]
            table_style = [
                ('SPAN', (0, 0), (1, 0)),
