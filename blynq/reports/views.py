...
def playlist_reports(request):
    screen_report_list, date_str_list, time_played_list, table_data = ([] for i in range(5))
    ...
    try:
        ...
        for obj in playlist_objects:
            all_playlists_dict[str(obj.playlist_id)] = {
                ...
                'time_played': 0, 'screens_played': [], 'playlist_total_time': obj.playlist_total_time}
        for single_date in date_range(start_date=start_date, end_date=end_date+timedelta(days=1)):
            ...
            media_analytics = MediaAnalytics.objects.filter(screen_id__in=screen_ids, playlist_ids__in=playlist_ids, date=single_date)
            for obj in media_analytics:
                playlist_dict = all_playlists_dict[str(obj.playlist_id)]
                # time_played_each_day = number of repetitions * each playlist time
                time_played_each_day += obj.count * playlist_dict['playlist_total_time']
                playlist_dict['num_of_repetitions'] += obj.count
                playlist_dict['time_played'] = playlist_dict['num_of_repetitions'] * playlist_dict['playlist_total_time']
            ...
            time_played_list.append(time_played_each_day)
